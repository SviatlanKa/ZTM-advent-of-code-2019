const wire1Str = `R75,D30,R83,U83,L12,D49,R71,U7,L72`
...
const wire2Str = `U62,R66,U55,R34,D71,R55,D58,R83`
    //`U98,R91,D20,R16,D67,R40,U7,R15,U6,R7`

    // `L996,U383,L962,U100,L836,D913,R621,U739,R976,D397,L262,D151,L12,U341,
...
const calculateSteps = (start, end) => {
    const temp = (start.x === end.x
...
function findCrossPath(path1, path2) {
    ...
    for (let i = 1; i < path1.length; i++) {
        ...
        for (let j = 1; j < path2.length; j++) {
            ...
            console.log(stepsPath2)

            if (start1.x === end1.x && start2.y === end2.y
                && inRange(start1.y, end1.y, start2.y)
                && inRange(start2.x, end2.x, start1.x)
            ) {
                console.log(Math.abs(start1.x), Math.abs(start2.y))
                crossPoints.push({
                    distance: Math.abs(start1.x) + Math.abs(start2.y),
                    steps: stepsPath1 + stepsPath2 - (Math.abs(end2.x) - Math.abs(start1.x) + Math.abs(start2.y))
                });
            };
            if (start1.y === end1.y && start2.x === end2.x
                ...
                console.log(Math.abs(start1.y), Math.abs(start2.x))
                crossPoints.push({
                    distance: Math.abs(start1.y) + Math.abs(start2.x),
                    steps: stepsPath1 + stepsPath2 - (Math.abs(start1.y) + Math.abs(start2.x) - Math.abs(end1.x))
                });
            }
...
console.log(crossPoints);
const distance = crossPoints.map(crossPoint => crossPoint.distance);
const steps = crossPoints.map(crossPoint => crossPoint.steps);
