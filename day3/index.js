    ...
    for (let i = 1; i < path1.length; i++) {
        ...
        for (let j = 1; j < path2.length; j++) {
            ...
            if (start1.x === end1.x && start2.y === end2.y
                ...
                const step1 = stepsPath1 - (Math.max(end1.y, start2.y) - Math.min(end1.y, start2.y));
                const step2 = stepsPath2 - (Math.max(start1.x, end2.x) - Math.min(start1.x, end2.x));
                crossPoints.push({
            ...
            if (start1.y === end1.y && start2.x === end2.x
                ...
                const step1 = stepsPath1 - (Math.max(end1.x, start2.x) - Math.min(end1.x, start2.x));
                const step2 = stepsPath2 - (Math.max(end2.y, start1.y) - Math.min(end2.y, start1.y));
                crossPoints.push({
